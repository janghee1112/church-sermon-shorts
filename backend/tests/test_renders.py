...
def test_banner_asset_alpha_and_safe_centered_layout(tmp_path):
    ...
    layout = calculate_banner_layout(1080, 1920, source_width, source_height, 0.368, 0.5, 0.790625)
    with Image.open(asset.path) as image:
        alpha_min, alpha_max = image.getchannel("A").getextrema()
    assert (source_width, source_height) == (1799, 361)
    assert (alpha_min, alpha_max) == (0, 255)
    assert (layout.width, layout.height, layout.x, layout.y) == (397, 80, 342, 1518)
    assert round(1920 * 0.806375) - layout.y == 30
